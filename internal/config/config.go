...
func destConf() error {
	...
	default:
		return fmt.Errorf("invalid/unknown metric destination (%s)", dest)
	}
...
func loadCOSIConfig() (string, string, string, error) {
	...
	if err != nil {
		return "", "", "", fmt.Errorf("unable to access cosi config: %w", err)
	}

	var cfg cosiConfig
	if err := json.Unmarshal(data, &cfg); err != nil {
		return "", "", "", fmt.Errorf("Unable to parse cosi config (%s): %w", cosiCfgFile, err)
	}

	if cfg.APIKey == "" {
		return "", "", "", fmt.Errorf("Missing API key, invalid cosi config (%s)", cosiCfgFile)
	}
	if cfg.APIApp == "" {
		return "", "", "", fmt.Errorf("Missing API app, invalid cosi config (%s)", cosiCfgFile)
	}
	if cfg.APIURL == "" {
		return "", "", "", fmt.Errorf("Missing API URL, invalid cosi config (%s)", cosiCfgFile)
	}
...
func logConfDir() error {
	...
	if dir == "" {
		return fmt.Errorf(errMsg+" (%s)", dir)
	}
	...
	if !fi.Mode().IsDir() {
		return fmt.Errorf(errMsg+" (%s) not a directory", dir)
	}
